...
class DatabaseManager {
    ...
    public static function guardarPago($payment_data) {
        ...
        $sql = "INSERT INTO pagos 
                (payment_id, status, amount, currency, description, qr_code, payer_email, external_reference, created_at)
                VALUES 
                (:payment_id, :status, :amount, :currency, :description, :qr_code, :payer_email, :external_reference, NOW())";
    ...
    public static function actualizarEstadoPago($payment_id, $nuevo_estado) {
        ...
        $sql = "UPDATE pagos SET status = :status, updated_at = NOW() WHERE payment_id = :payment_id";
        $stmt = $db->prepare($sql);
